...
const CardStack = () => {
  ...
  return (
    <section
      ref={sectionRef}
      className="relative w-full bg-ice-snow"
      style={{ minHeight: `${(cards.length + 1) * 100}vh` }}
    >
      {/* Section Header */}
      <div className="absolute top-0 left-0 right-0 py-12 md:py-16 text-center z-10">
        <h2 className="font-display text-headline text-glacier-deep">
          {cardStackConfig.sectionTitle}
        </h2>
        <p className="font-body text-sm text-glacier-primary uppercase tracking-[0.2em] mt-4">
      ...
      >
        <div className="relative w-full max-w-4xl mx-auto px-6 md:px-8 aspect-[4/3]">
          {cards.map((card, index) => (
            ...
            >
              <div className="relative overflow-hidden rounded-3xl shadow-deep bg-ice-white h-full">
                {/* Image */}
                <div className="absolute inset-0 overflow-hidden">
                  ...
                  />
                  {/* Image Overlay */}
                  <div className="absolute inset-0 bg-gradient-to-t from-glacier-deep/60 via-transparent to-transparent" />
                </div>

                {/* Card Content */}
                <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8">
                  <h3 className="font-display text-2xl md:text-3xl text-ice-white mb-2">
                    {card.title}
                  </h3>
                  <p className="font-body text-sm text-ice-white/70">
                    {card.description}
                  </p>
                </div>

                {/* Card Number */}
                <div className="absolute top-4 right-4 w-8 h-8 rounded-full bg-ice-white/20 backdrop-blur-sm flex items-center justify-center">
                  <span className="font-body text-xs text-ice-white">
